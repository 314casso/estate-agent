...
class WPService(object):
    ...
    def render_post_category_depricated(self, estate):
        result = []        
    ...
    def render_custom_fields(self, estate):        
        ...
        fields['microdistrict'] = estate.microdistrict.name
        fields['bidgarea'] = float(estate.basic_bidg.total_area) if estate.basic_bidg else None 
        fields['steadarea'] = float(estate.basic_stead.total_area_sotka) if estate.basic_stead else None
        ...
        fields['map_lat'] = u'%s' % estate.latitude
        fields['map_lng'] = u'%s' % estate.longitude
        if estate.links:
            for link in estate.links.all():
                if 'youtu' in link.url:            
                    fields['youtube'] = u'%s' % link.url
