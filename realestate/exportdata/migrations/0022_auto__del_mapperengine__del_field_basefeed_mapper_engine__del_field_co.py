...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'ContentTypeMapper.mapper_engine'
        db.delete_column('exportdata_contenttypemapper', 'mapper_engine_id')

        # Adding field 'ContentTypeMapper.feed_engine'
        db.add_column('exportdata_contenttypemapper', 'feed_engine',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['exportdata.FeedEngine']),
                      keep_default=False)

        # Adding unique constraint on 'ContentTypeMapper', fields ['feed_engine', 'content_type']
        db.create_unique('exportdata_contenttypemapper', ['feed_engine_id', 'content_type_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'ContentTypeMapper', fields ['feed_engine', 'content_type']
        db.delete_unique('exportdata_contenttypemapper', ['feed_engine_id', 'content_type_id'])

        # Adding model 'MapperEngine'
        db.create_table('exportdata_mapperengine', (
            ('engine', self.gf('django.db.models.fields.CharField')(default='AVITO', max_length=15)),
        ...
        # User chose to not deal with backwards NULL issues for 'BaseFeed.mapper_engine'
        raise RuntimeError("Cannot reverse this migration. 'BaseFeed.mapper_engine' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'BaseFeed.mapper_engine'
        ...
        # User chose to not deal with backwards NULL issues for 'ContentTypeMapper.mapper_engine'
        raise RuntimeError("Cannot reverse this migration. 'ContentTypeMapper.mapper_engine' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'ContentTypeMapper.mapper_engine'
        db.add_column('exportdata_contenttypemapper', 'mapper_engine',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['exportdata.MapperEngine']),
                      keep_default=False)

        # Deleting field 'ContentTypeMapper.feed_engine'
        db.delete_column('exportdata_contenttypemapper', 'feed_engine_id')
    ...
    models = {
        ...
        'exportdata.marketingcampaign': {
            ...
            'start_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2016, 7, 9, 0, 0)'})
        },
