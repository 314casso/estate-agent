...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Office.region'
        db.delete_column('estatebase_office', 'region_id')

        # Adding M2M table for field regions on 'Office'
        db.create_table('estatebase_office_regions', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('office', models.ForeignKey(orm['estatebase.office'], null=False)),
            ('region', models.ForeignKey(orm['estatebase.region'], null=False))
        ))
        db.create_unique('estatebase_office_regions', ['office_id', 'region_id'])


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Office.region'
        raise RuntimeError("Cannot reverse this migration. 'Office.region' and its values cannot be restored.")
        # Removing M2M table for field regions on 'Office'
        db.delete_table('estatebase_office_regions')
