...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'GenericLink.order'
        db.delete_column('estatebase_genericlink', 'order')


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'GenericLink.order'
        raise RuntimeError("Cannot reverse this migration. 'GenericLink.order' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'GenericLink.order'
        db.add_column('estatebase_genericlink', 'order',
                      self.gf('django.db.models.fields.PositiveIntegerField')(unique=True, blank=True),
                      keep_default=False)
